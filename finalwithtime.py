...
def calculate_strength(password):
    ...
    end_time = time.time()
    elapsed_time = end_time - start_time
    return int(strength), elapsed_time
...
def check_password():
    ...
    cracked_password, elapsed_time = crack_password(hash_value, password_list)
    if cracked_password:
    ...
    time_label.configure(text="Elapsed time: {:.6f} seconds".format(elapsed_time))
...
def check_password_strength():
    password = password_entry.get()
    strength, elapsed_time = calculate_strength(password)
...
time_label = ttk.Label(output_pane, text="", style='TLabel')
